...
D cons(D a, D d) {
  printf("CONS: "); princ(a); putchar(' '); princ(d); putchar('\n');
  ++C;
  C->car= a;
  C->cdr= d;
  return (D)C;
...
D apply(D f, D x, D env) {
  ...
  if (f==LAMBDA) return eval( // TODO: quote ?
...
int main(int argc, char** argv) {
  ...
  CAR= atom("car"); car(CAR)= CAR;
  CDR= atom("cdr"); car(CDR)= CDR;
  CONS= atom("cons"); car(CONS)= CONS;
  EQ= atom("eq"); car(EQ)= EQ; car(atom("="))= EQ;
  PLUS= atom("+"); car(PLUS)= PLUS; car(atom("plus"))= PLUS;
